...
export const createThought = async (_req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate({ _id: _req.body.idUser }, { $addToSet: { thoughts: { _idThought: thought._id, thoughtText: thought.thoughtText } } }, { new: true });
        if (!user) {
            res.status(404).json({
...
export const createReaction = async (_req, res) => {
    try {
        const thought = await Thought.findByIdAndUpdate({ _id: _req.params.thoughtId }, { $addToSet: { reactions: _req.body } }, { new: true });
        if (!thought) {
...
export const deleteReaction = async (_req, res) => {
    try {
        const thought = await Thought.findByIdAndUpdate({ _id: _req.params.thoughtId }, { $pull: { reactions: _req.body } }, { new: true });
        if (!thought) {
            res.status(404).json({ msg: "There is a mistake" });
        }
        res.json({
